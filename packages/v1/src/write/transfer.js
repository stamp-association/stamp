...
function trade({ state, action }) {
  const targetBalance = state.balances[action.input.target] ? state.balances[action.input.target] : 0
  return {
    state: {
      ...state,
      balances: {
...
function validate({ state, action }) {
  ...
  if (!state.balances[action.caller] || typeof state.balances[action.caller] !== 'number' || state.balances[action.caller] < action.input.qty) {
