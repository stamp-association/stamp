...
test('should transfer ownership', async () => {
  const STAMP = 1 * 1e12
  const result = await [{ caller: TOM, input: { function: 'transfer', qty: 5 * STAMP, target: JUSTIN } }].reduce(handle, { balances: { [TOM]: 5 * STAMP } })
  assert.equal(path(['state', 'balances', JUSTIN], result), 5 * STAMP)
})

test('should transfer half', async () => {
  const STAMP = 1 * 1e12
  const result = await [{ caller: TOM, input: { function: 'transfer', qty: 2 * STAMP, target: JUSTIN } }].reduce(handle, { balances: { [TOM]: 4 * STAMP } })
...
test('should not transfer if caller has no balance', async () => {
  const STAMP = 1 * 1e12
  try {
    await [{ caller: TOM, input: { function: 'transfer', qty: 2 * STAMP, target: JUSTIN } }].reduce(handle, { balances: {} })
  } catch (e) {
...
test('caller can not be target', async () => {
  const STAMP = 1 * 1e12
  try {
    await handle({ balances: {} }, { caller: TOM, input: { function: 'transfer', qty: 2 * STAMP, target: TOM } })
  } catch (e) {
...
test('qty must be an integer', async () => {
  const STAMP = 1 * 1e12
  try {
    await handle({}, { caller: TOM, input: { function: 'transfer', target: JUSTIN, qty: '10000' } })
  } catch (e) {
