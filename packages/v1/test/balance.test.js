...
test('should return balance for TOM', async () => {
  const STAMP = 1 * 1e12
  const { result } = await handle({ balances: { [TOM]: 1000 * STAMP } }, { caller: TOM, input: { function: 'balance' } })
  assert.equal(result.balance, 1000 * STAMP)
})

test('should return balance for JUSTIN', async () => {
  const STAMP = 1 * 1e12
  const { result } = await handle({ balances: { [TOM]: 1000 * STAMP, [JUSTIN]: 500 * STAMP } }, { caller: TOM, input: { target: JUSTIN, function: 'balance' } })
  assert.equal(result.balance, 500 * STAMP)
})

test('should return balance 0 for JUSTIN', async () => {
  const STAMP = 1 * 1e12
  const { result } = await handle({ balances: { [TOM]: 1000 * STAMP } }, { caller: TOM, input: { target: JUSTIN, function: 'balance' } })
